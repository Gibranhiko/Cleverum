...
import { flatProducts, formatOrder, formatProducts } from "../../utils/order";
import { confirmation } from "../confirmation.flow";
import { welcome } from "../welcome.flow";

export const createFoodFlow = (products) => {
  const flattenProds = flatProducts(products);
  const optionsMsg = formatProducts(flattenProds);
  ...
  return addKeyword(EVENTS.ACTION)
    .addAnswer(
      ...
      async (ctx, { fallBack, flowDynamic, state, gotoFlow }) => {
        ...
        const selectedProduct = flattenProds[selection];
    ...
    .addAnswer(
      ...
      async (ctx, { gotoFlow, fallBack, flowDynamic, state }) => {
        if (ctx.body === "1") {
          ...
          await flowDynamic(`Tu pedido actual es:\n${orderDetails.join("\n").trim()}`);
          await flowDynamic(
